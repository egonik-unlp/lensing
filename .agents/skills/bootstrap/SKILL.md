...
Qdrant first (default `http://localhost:6333`; ask if theirs differs):
...
3. **Write `pipeline.toml`** — one `[embedding]`, one `[source]`, and sinks in
   ...
   [[sink]]
   kind = "qdrant"
   url = "http://localhost:6333"          # api_key = "..." for a remote/cloud Qdrant
   ```
...
## Phase 3 — Ingestion: how their "item equivalent" arrives
...
curl -s localhost:8080/api/collections/validate -H content-type:application/json \
...
```sh
curl -s localhost:8080/api/representations -H content-type:application/json \
  -d '{"method": "pca", "latent": 32}'          # method: pca | autoencoder | sparse_ae
curl -s localhost:8080/api/representations          # list; poll build status
```
...
- **Training always goes through the experiment helpers.** Beyond the first
  ...
  **information-capture-analyst** (SAE interpretability) — and keeps the
  empirical record (`experiments/PROJECT-FACTS.md`) coherent. Do not hand-launch ad-hoc
