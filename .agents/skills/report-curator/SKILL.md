---
name: report-curator
description: Maintain the living scientific report in both its registers (docs/experiments.tex technical + docs/experiments-v2.tex plain-language + docs/figures/make_figures.py → docs/experiments.pdf + docs/experiments-v2.pdf) consolidating the experiments/*.md campaign reports. Spawns the report-curator agent. Use when the user wants to update or sync the experiment PDFs, fold new experiment reports into the documents, add a figure (heatmaps for parameter scans), add or extend the theory/background section, refresh the leaderboard, polish the prose, or rebuild the report.
user-invocable: true
...
|---|---|---|
| **sync** (default) | Detect `experiments/*.md` reports not yet reflected in each document, fold each in additively into BOTH registers (section + transcribed tables + figures + leaderboard/abstract refresh), rebuild both PDFs | proceeds freely |
