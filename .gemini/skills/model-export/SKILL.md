...
every target-prediction model this repo trains.
...
```bash
curl -fsS -OJ http://localhost:8080/api/models/<name>/export    # -> <name>-export.tar.gz
...
<name>/
  ...
  pca_components.f32     PCA basis, row-major float32[dims, 1536]
...
The embedding (`1536`-dim) is supplied by the caller — it comes
...
`POST http://localhost:8080/api/models/<name>/predict` (server featurizer + native
