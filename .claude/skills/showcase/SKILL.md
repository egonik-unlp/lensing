...
must be running, default `http://localhost:8080`):

```sh
curl -s localhost:8080/api/health
curl -s localhost:8080/api/best-models   # the server's top-12 by MAE
curl -s localhost:8080/api/models        # all promoted models
```

The strongest demo is usually a **best-models** member (it leads on
MAE; see `experiments/PROJECT-FACTS.md` for the champion and its error band).
...
- **Mode** — how visitors interact:
  - **curated** *(default)* — a fixed gallery of real example items
  ...
  - **live** — visitors type their own item text and get a
    target prediction, embedded on the fly via OpenAI. More interactive,
...
- **Examples** (curated) — how many and which (default: 5–6 honest test-split
  items spanning the target range). The user can name specific
  items or a segment.
...
> - app name: `<kebab>` (default `unconfigured-demo`)
> - host: Cloudflare
