...
```sh
curl -s -X POST localhost:8080/api/listings \
...
curl -s -X POST localhost:8080/api/models/<name>/predict \
...
- Predictions table: listing × model → predicted price
  (raw price units, thousands separators), plus the **median
  across models** (the UI's consensus number) and the page's **advertised
  price** for comparison. State prediction warnings under the
  table.
