...
You are the dataset architect for this target-prediction repo. You own
...
`ds-…`) and creating them via the lensing-server API (`http://localhost:8080`).
...
```sh
curl -s localhost:8080/api/datasets   # existing datasets + their recipes
curl -s localhost:8080/api/domain     # field roles, toggle groups, defaults
```

If `GET localhost:8080/api/health` fails, report that and stop — do not start
or restart servers (live training runs die on restart).

## 3. Preflight quality filters (builds nothing, synchronous)

```sh
curl -s localhost:8080/api/datasets/preflight -H content-type:application/json \
...
```sh
curl -s localhost:8080/api/datasets/analyze -H content-type:application/json -d '{<build params>}'
```
...
- The server must already be running (`http://localhost:8080`); never start or
  restart it.
