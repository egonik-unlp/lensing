...
- The Qdrant corpus (`{{collection}}` at `{{qdrant_url}}`) is external;
  manual entries live in `{{manual_collection}}`.
...
- **listing-generator** (agent) — URL → manual {{entity_noun}} → predictions
